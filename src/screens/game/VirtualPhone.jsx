...
const PHONE_APPS = [
  { id: "chat", label: "聊天", subtitle: "未读 3" },
  { id: "journal", label: "日记", subtitle: "昨日残响" },
  { id: "wallet", label: "钱包", subtitle: "账单同步" },
  { id: "jobs", label: "招聘", subtitle: "新职位" },
  { id: "settings", label: "设置", subtitle: "系统偏好" },
];
...
function playPhoneUiSound(kind) {
  ...
  gain.gain.exponentialRampToValueAtTime(PHONE_AUDIO_SETTINGS.volume, now + 0.02);
  gain.gain.exponentialRampToValueAtTime(0.0001, now + (kind === "close" ? 0.16 : 0.2));
...
function PhoneAppIcon({ app, onOpen, isPressing }) {
  return (
    ...
    >
      <span className={`virtual-phone__app-icon virtual-phone__app-icon--${app.id}`}>
        <PhoneIcon kind={app.id} />
        {app.id === "chat" ? <span className="virtual-phone__app-badge">3</span> : null}
      </span>
...
function PhoneIcon({ kind }) {
  ...
  if (kind === "chat") {
    return (
      <svg {...commonProps}>
        <path d="M6.5 8.2C6.5 6.7 7.8 5.5 9.4 5.5H14.7C16.3 5.5 17.5 6.7 17.5 8.2V11.8C17.5 13.3 16.3 14.5 14.7 14.5H11.3L8.6 17V14.5H9.3C7.8 14.5 6.5 13.3 6.5 11.8V8.2Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  if (kind === "journal") {
    return (
      <svg {...commonProps}>
        <path d="M8 6.2H15.2C16 6.2 16.6 6.8 16.6 7.6V17.6H8.7C7.8 17.6 7.2 17 7.2 16.1V7C7.2 6.6 7.5 6.2 8 6.2Z" stroke="currentColor" strokeWidth="1.5" />
        <path d="M9.8 9.3H13.8M9.8 12H13.8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  if (kind === "wallet") {
    return (
      <svg {...commonProps}>
        <path d="M7.4 8H15.8C16.7 8 17.4 8.7 17.4 9.6V15.2C17.4 16.1 16.7 16.8 15.8 16.8H8.2C7.1 16.8 6.2 15.9 6.2 14.8V9.2C6.2 8.5 6.7 8 7.4 8Z" stroke="currentColor" strokeWidth="1.5" />
        <path d="M14.3 12.4H17.2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        <circle cx="13.2" cy="12.4" r="0.8" fill="currentColor" />
      </svg>
    );
  }
  ...
  if (kind === "jobs") {
    return (
      <svg {...commonProps}>
        <path d="M9.2 7.3H14.8C15.6 7.3 16.2 7.9 16.2 8.7V16.8H7.8V8.7C7.8 7.9 8.4 7.3 9.2 7.3Z" stroke="currentColor" strokeWidth="1.5" />
        <path d="M10.1 7.3V6.6C10.1 6 10.6 5.5 11.2 5.5H12.8C13.4 5.5 13.9 6 13.9 6.6V7.3" stroke="currentColor" strokeWidth="1.5" />
  ...
  if (kind === "settings") {
    return (
      <svg {...commonProps}>
        <path d="M12 9.3A2.7 2.7 0 1 0 12 14.7A2.7 2.7 0 1 0 12 9.3Z" stroke="currentColor" strokeWidth="1.5" />
        <path d="M12 5.5V7M12 17V18.5M17 12H18.5M5.5 12H7M15.9 8.1L14.9 9.1M9.1 14.9L8.1 15.9M15.9 15.9L14.9 14.9M9.1 9.1L8.1 8.1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  return (
    <section className="virtual-phone__home">
      <header className="virtual-phone__hero">
        ...
        <h2 className="virtual-phone__title">桌面</h2>
      </header>
  ...
  return (
    <section className="virtual-phone__app-screen">
      ...
      <div className="virtual-phone__placeholder">
        <span className={`virtual-phone__placeholder-icon virtual-phone__placeholder-icon--${appId}`}>
          <PhoneIcon kind={appId} />
        </span>
